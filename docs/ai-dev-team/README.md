...
| 軽量モデルルーティング方針を provider-config に正式反映 | 実装済み（v0.1.9）— API実行なし |
| Cloud Run PM Agent foundation | 実装済み（v0.2.0）— deploy なし・API実行なし |
| HTTP dry-run intake | 実装済み（v0.2.1）— HTTP サーバー実装・deployなし |
| HTTP request fixtures & local smoke client | 実装済み（v0.2.2）— fixture 3種・client 整備・deployなし |
| Cloud Run Launch Pack MAX | 実装済み（v0.2.3）— Dockerfile/template/preflight/docs/smoke 整備・deployなし |
| Deploy Execution & Runtime Ops Pack | 実装済み（v0.3.0）— deploy 承認・最終確認・runtime 運用・webhook 設計・コマンドパック整備 |
| First Cloud Run Deploy Execution | v0.4.0 — じゅんやさんが Cloud Shell で実行（Human Approval 必要） |
| deploy 結果記録 | v0.4.1 — テンプレート実装済み（deploy 後に記録） |
| Cloud Run URL smoke 記録 | v0.4.1 — deploy 後の URL smoke 記録テンプレート |
| n8n 初回接続・webhook 接続結果記録 | v0.4.2 — 接続設定・結果テンプレート実装済み |
| Secret Manager 準備ガイド | v0.4.2 — readiness チェックリスト実装済み |
| 本番移行メモ | v0.4.2 — Go/No-Go / rollback / monitoring 計画実装済み |
