...
const tool = require('../tools/gemini-draft-expansion-pack.js');

const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json'), 'utf8'));

console.log('=== gemini-draft-expansion-pack smoke ===');

assert.ok(compareVersion(pkg.version, '5.0.0') >= 0);
console.log('  PASS: package version 5.0.0 or later');

assert.ok(pkg.scripts['smoke:gemini-draft-expansion-pack']);
console.log('  PASS: script exists');

assert.ok(fs.existsSync(path.join(__dirname, '../docs/ai-dev-team/kosame-dev-orchestra-v4.8.0-release-record.md')));
console.log('  PASS: release record exists');

assert.ok(fs.existsSync(path.join(__dirname, '../fixtures/gemini-draft-expansion-pack.sample.json')));
console.log('  PASS: fixture exists');

assert.strictEqual(tool.TOOL_META.version, '4.8.0');
...
console.log('PASS: gemini-draft-expansion-pack');
