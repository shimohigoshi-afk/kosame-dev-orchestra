...
const tool = require('../tools/kosame-approval-compression.js');

console.log('=== kosame-approval-compression smoke ===');

const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json'), 'utf8'));
const docPath = path.join(__dirname, '../docs/ai-dev-team/kosame-dev-orchestra-v4.3.0-release-record.md');
const fixturePath = path.join(__dirname, '../fixtures/kosame-approval-compression.sample.json');

assert.strictEqual(pkg.version, '4.5.0', 'package version 4.5.0');
console.log('  PASS: package version 4.5.0');

assert.ok(pkg.scripts['smoke:kosame-approval-compression'], 'script exists: smoke:kosame-approval-compression');
...
assert.strictEqual(tool.TOOL_META.version, '4.3.0', 'tool meta version');
...
console.log('PASS: kosame-approval-compression');
