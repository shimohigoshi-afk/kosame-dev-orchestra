...
const tool = require('../tools/kosame-daily-operation-snapshot.js');

console.log('=== kosame-daily-operation-snapshot smoke ===');

const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json'), 'utf8'));
const docPath = path.join(__dirname, '../docs/ai-dev-team/kosame-dev-orchestra-v4.2.0-release-record.md');
const fixturePath = path.join(__dirname, '../fixtures/kosame-daily-operation-snapshot.sample.json');

assert.strictEqual(pkg.version, '4.5.0', 'package version 4.5.0');
console.log('  PASS: package version 4.5.0');

assert.ok(pkg.scripts['smoke:kosame-daily-operation-snapshot'], 'script exists: smoke:kosame-daily-operation-snapshot');
...
assert.strictEqual(tool.TOOL_META.version, '4.2.0', 'tool meta version');
...
console.log('PASS: kosame-daily-operation-snapshot');
