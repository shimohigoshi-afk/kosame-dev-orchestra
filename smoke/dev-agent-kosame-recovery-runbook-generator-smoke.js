...
const tool = require('../tools/kosame-recovery-runbook-generator.js');

console.log('=== kosame-recovery-runbook-generator smoke ===');

const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json'), 'utf8'));
const docPath = path.join(__dirname, '../docs/ai-dev-team/kosame-dev-orchestra-v4.4.0-release-record.md');
const fixturePath = path.join(__dirname, '../fixtures/kosame-recovery-runbook-generator.sample.json');

assert.strictEqual(pkg.version, '4.5.0', 'package version 4.5.0');
console.log('  PASS: package version 4.5.0');

assert.ok(pkg.scripts['smoke:kosame-recovery-runbook-generator'], 'script exists: smoke:kosame-recovery-runbook-generator');
...
assert.strictEqual(tool.TOOL_META.version, '4.4.0', 'tool meta version');
...
console.log('PASS: kosame-recovery-runbook-generator');
