...
const tool = require('../tools/multi-agent-practical-routing-console.js');

const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json'), 'utf8'));

console.log('=== multi-agent-practical-routing-console smoke ===');

assert.ok(compareVersion(pkg.version, '5.0.0') >= 0);
console.log('  PASS: package version 5.0.0 or later');

assert.ok(pkg.scripts['smoke:multi-agent-practical-routing-console']);
console.log('  PASS: script exists');

assert.ok(fs.existsSync(path.join(__dirname, '../docs/ai-dev-team/kosame-dev-orchestra-v5.0.0-release-record.md')));
console.log('  PASS: release record exists');

assert.ok(fs.existsSync(path.join(__dirname, '../fixtures/multi-agent-practical-routing-console.sample.json')));
console.log('  PASS: fixture exists');

assert.strictEqual(tool.TOOL_META.version, '5.0.0');
...
console.log('PASS: multi-agent-practical-routing-console');
