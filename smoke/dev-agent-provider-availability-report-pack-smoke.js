...
const tool = require('../tools/provider-availability-report-pack.js');

const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json'), 'utf8'));

console.log('=== provider-availability-report-pack smoke ===');

assert.ok(compareVersion(pkg.version, '5.0.0') >= 0);
console.log('  PASS: package version 5.0.0 or later');

assert.ok(pkg.scripts['smoke:provider-availability-report-pack']);
console.log('  PASS: script exists');

assert.ok(fs.existsSync(path.join(__dirname, '../docs/ai-dev-team/kosame-dev-orchestra-v4.7.0-release-record.md')));
console.log('  PASS: release record exists');

assert.ok(fs.existsSync(path.join(__dirname, '../fixtures/provider-availability-report-pack.sample.json')));
console.log('  PASS: fixture exists');

assert.strictEqual(tool.TOOL_META.version, '4.7.0');
...
console.log('PASS: provider-availability-report-pack');
