...
async function main() {
  ...
  if (!geminiSrc) { fail('kosame-gemini.js exists', 'file not found'); }
    ...
    checkContains('gemini: model is gemini-2.0-flash', geminiSrc, "gemini-2.0-flash");
    checkNotContains('gemini: old model gemini-1.5-flash removed', geminiSrc, 'gemini-1.5-flash');
  ...
  if (pkg) {
    ...
    if (String(pkg.version || '').includes('113.3.22')) ok('package.json version: 113.3.22');
    else fail('package.json version: 113.3.22', `got ${pkg.version}`);
  }
