...
async function main() {
  ...
  assert.ok(implRes.body.instruction.includes('claude -p'), 'claude_code instruction must have claude -p');
  ...
  assert.equal(civilRes.body.route, 'deepseek_grok',  'smoke task must route to deepseek_grok');
  ...
  console.log('  PASS: POST /api/dev-os → deepseek_grok (smoke追加して) + Task Pack JSON');
  ...
  const blockRes = await withServer(createDevOsServer, (s) =>
    reqServer(s, '/api/dev-os', { task: 'transcriberのsmoke追加して' })
  );
