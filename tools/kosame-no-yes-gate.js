...
function evaluateNoYesGate(input = {}) {
  ...
  if (safety.matched) {
    return {
      ...
      codexYesHellGuard: executionHostInfo.codexYesHellGuard,
      codexAutoApproveMode: executionHostInfo.codexAutoApproveMode,
      userYesRequired: executionHostInfo.userYesRequired,
      safetyStopGuard: executionHostInfo.safetyStopGuard,
      source,
...
function summarizeNoYesGateState(state = {}) {
  return [
    ...
    `interactivePromptBlocked=${state.interactivePromptBlocked ? 'true' : 'false'}`,
